...
export const FormOperacionalEmprestimoVeiculo = () => {
    ...
    const [avalistaFiltrado, setAvalistaFiltrado] = useState([]);

    const [buscaAvalista, setBuscaAvalista] = useState(false);

    const [avalista, setAvalista] = useState([]);

    const [nomeAvalista, setNomeAvalista] = useState('');

    const [cnpjCpfAvalista, setCnpjCpfAvalista] = useState('');
    ...
    //atualizar veiculo emprestimo
    const atualizarVeiculoEmprestimo = async () => {
        const dadosEmprestimo = ref.current;
    ...
    const formataCpfCnpjAvalista = () => {
        const dadosCredor = ref.current;
        let vCpfCnpj = dadosCredor.cnpjCpfAvalista.value;
        setCnpjCpfAvalista(cpfCnpjMask(vCpfCnpj));
    };

    const formataCpfCnpjCredor = () => {
        const dadosCredor = ref.current;
    ...
    const buscaClienteCredor = async () => {
        const dadosCredor = ref.current;
    ...
    const buscaClienteAvalista = async () => {
        const dadosCredor = ref.current;
        ...
        await apiFactoring
            .post(
                '/busca-cliente',
                {
                    cnpjCpf: dadosCredor.cnpjCpfAvalista.value,
                },
                {
                    headers: {
                        'x-access-token': localStorage.getItem('user'),
            ...
            .then(({ data }) => {
                if (data.length > 0) {
                    data.map((dados) => {
                        setNomeAvalista(dados.nome);
                    });
                } else {
                    dadosCredor.nomeAvalista.value = '';
                }
            })
            .catch(({ data }) => {
                toast.error(data);
    ...
    const FiltraAvalista = (busca) => {
        let avalistaF = [];

        avalistaF = avalista.filter((A) =>
            A.nome.toUpperCase().includes(busca.toUpperCase() || A.nome != ' ')
        );

        setAvalistaFiltrado(avalistaF);
        if (avalistaF.length == 0) {
            setBuscaAvalista(false);
        } else {
            setBuscaAvalista(true);
        }
    };
    ...
    const imprimir = () => {
        ...
        win.document.write('<body>');
        win.document.write(
            '<table border="0" style="width: 300px; font-size: 10; font-family: courier; font-weight: 550;">'
        );
        ...
        win.document.write('</td>');
        win.document.write('<td style="text-align: right; width: 130px">Pago');
        win.document.write('</td>');
        win.document.write('</tr>');
        arrayParcelas.map((item) => {
            win.document.write('<tr>');
                            ...
                            <div className="boxRow">
                                <div className="boxCol">
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        value={cnpjCpfAvalista || ''}
                                        placeholder=""
                                        autoComplete="off"
                                        onChange={(e) => {
                                            setCnpjCpfAvalista(e.target.value);
                                        }}
                                        onBlur={formataCpfCnpjAvalista}
                                        onKeyDown={(e) =>
                                            keyDown(e, 'inputAvalista')
                                        }
                                        onFocus={(e) => {
                                            setFormBuscaDireto(false);
                                        }}
                                ...
                                <div className="boxCol">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            FiltraAvalista(e.target.value);
                                            setNomeAvalista(e.target.value);
                                        }}
                                    />
                                    {buscaAvalista == true && (
                                        <BuscaAvalistaNome
                                            avalistaFiltrado={avalistaFiltrado}
                                            setCnpjCpfAvalista={
                                                setCnpjCpfAvalista
                                            }
                                            setBuscaAvalista={setBuscaAvalista}
                                        />
                                    )}
                                </div>
                            </div>
